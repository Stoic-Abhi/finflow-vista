...
import { Mail, Lock, Eye, EyeOff, Chrome, Github, Smartphone } from 'lucide-react';
import { Button } from '../ui/Button';
import { Input } from '../ui/Input';
import { useAuthStore } from '../../store/authStore';
import toast from 'react-hot-toast';

export const LoginForm = ({ onSwitchToRegister, onForgotPassword }) => {
  const [showPassword, setShowPassword] = useState(false);
  const { login, loginWithOAuth, loginWithPhone, isLoading, error, clearError } = useAuthStore();
  ...
  const handleOAuthLogin = async (provider) => {
    try {
      await loginWithOAuth(provider, 'mock-oauth-token');
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex items-center justify-between animate-slideIn">
          ...
          <button
            ...
            className="text-sm text-blue-600 hover:text-blue-700 font-medium transition-colors button-hover"
          >
            Forgot password?
          </button>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 animate-slideIn">
          <Button 
            ...
            disabled={isLoading}
            className="h-12 button-hover card-hover"
          >
        ...
        <div className="text-center animate-slideIn">
          <p className="text-xs text-gray-500 mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
            <strong>Demo credentials:</strong> <span className="font-mono">demo@example.com</span> / <span className="font-mono">password123</span>
          </p>
        </div>
